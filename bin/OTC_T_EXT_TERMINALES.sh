...
# sh -x /home/nae108834/RGenerator/reportes/Ext_Terminales/bin/OTC_T_EXT_TERMINALES.sh 20231106
...
beeline -u $VAL_CADENA_JDBC -n $VAL_USER \
	--hiveconf tez.queue.name=$VAL_QUEUE \
	--hivevar val_fecha_formato=${VAL_FECHA_FORMATO} \
	--hivevar val_fecha_ini=${VAL_FECHA_INI} \
	--hivevar val_dia_uno=${VAL_DIA_UNO} \
	--hivevar fecha_antes_ayer=${VAL_FECHA_FORMATO_PRE} \
	--hivevar ESQUEMA_TMP=${VAL_ESQUEMA_TMP} \
	--hivevar ESQUEMA_REP=${VAL_ESQUEMA_REP} \
	-f ${VAL_RUTA}/sql/otc_t_ext_terminales.sql 2>&1 &>> $VAL_LOG

# Verificacion de ejecucion BEELINE
if [ $? -eq 0 ]; then
	echo "EXITO en  ejecucion proceso BEELINE, ETAPA ${ETAPA}" 2>&1 &>> $VAL_LOG
	else
	echo "Fallo al ejecutar script desde BEELINE ETAPA ${ETAPA}" 2>&1 &>> $VAL_LOG
	exit 1
fi	
...
if [ "$ETAPA" = "4" ]; then
echo "=======================================================================================================" 2>&1 &>> $VAL_LOG
echo "==== ETAPA 4: Lee tabla ${VAL_ESQUEMA_REP}.otc_t_ext_terminales_ajst y genera archivo txt en ruta output ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
echo "=======================================================================================================" 2>&1 &>> $VAL_LOG
...
error_spark=`egrep 'Error PySpark:|An error occurred|ERROR FileFormatWriter:|Traceback|SyntaxError|UnicodeDecodeError|AnalysisException:|NameError:|IndentationError:|Permission denied:|ValueError:|error:|unrecognized arguments:|No such file or directory|Failed to connect|Could not open client' $VAL_LOG | wc -l`

--#################################################################
--# Requerimiento:     BIGD-410 Correcciones Ext. Terminales      #
--# Cliente:           Telefonica                                 #
--# Elaborado por:     Softconsulting S.A.                        #
--# Script:            otc_t_ext_terminales.sql                  #
--# Fecha de creacion: 2023/11/23                                 #
...
SET tez.am.resource.memory.mb=8192; --(8GB) 

SET hive.tez.container.SIZE=8192; -- (8GB) 

SET parquet.compression=GZIP; 

-- N1: Tabla Temporal con terminales de notas de credito
DROP TABLE IF EXISTS ${ESQUEMA_TMP}.tmp_terminales_simcards_nc;
CREATE EXTERNAL TABLE ${ESQUEMA_TMP}.tmp_terminales_simcards_nc
STORED AS PARQUET 
...
--N2: Tabla Temporal de terminales con factura, es decir diferentes de notas de credito
DROP TABLE IF EXISTS ${ESQUEMA_TMP}.tmp_terminales_simcards_factura;
CREATE EXTERNAL TABLE ${ESQUEMA_TMP}.tmp_terminales_simcards_factura
STORED AS PARQUET 
TBLPROPERTIES ('external.table.purge'='true')
AS
SELECT
...
END) AS tipo_doc_cliente
    ...
    LEFT JOIN ${ESQUEMA_TMP}.tmp_terminales_simcards_nc b
...
--N3: TABLA con la union de terminales NC y facturas
DROP TABLE IF EXISTS ${ESQUEMA_TMP}.tmp_terminales_simcards;
CREATE EXTERNAL TABLE ${ESQUEMA_TMP}.tmp_terminales_simcards
STORED AS PARQUET 
TBLPROPERTIES ('external.table.purge'='true')
AS
SELECT
...
FROM
	...
	,  id_hash
    ,  aplica_comision
    FROM ${ESQUEMA_TMP}.tmp_terminales_simcards_nc
UNION ALL
	SELECT 
    fecha_proceso
	,  fecha_factura
	...
	,  id_hash
    ,  aplica_comision
    FROM ${ESQUEMA_TMP}.tmp_terminales_simcards_factura) a
...
--N4: TABLA PREVIA PARA REPORTE DE EXTRACTOR DE TERMINALES
DROP TABLE IF EXISTS ${ESQUEMA_TMP}.otc_t_ext_terminales_ajst_tmp;
CREATE EXTERNAL TABLE ${ESQUEMA_TMP}.otc_t_ext_terminales_ajst_tmp
STORED AS PARQUET 
TBLPROPERTIES ('external.table.purge'='true')
AS
SELECT
...
FROM
	${ESQUEMA_TMP}.tmp_terminales_simcards tsim
UNION ALL
SELECT
    ...
    ,  t.aplica_comision 
FROM
	${ESQUEMA_TMP}.tmp_terminales_simcards t
INNER JOIN 
...
--N5 Query para la generacion de tabla final para inclusion de SELECT DISTINCT
DROP TABLE IF EXISTS ${ESQUEMA_REP}.otc_t_ext_terminales_ajst;
CREATE EXTERNAL TABLE ${ESQUEMA_REP}.otc_t_ext_terminales_ajst
STORED AS PARQUET 
TBLPROPERTIES ('external.table.purge'='true')
AS
SELECT DISTINCT 
	...
	, id_hash
	, aplica_comision
FROM ${ESQUEMA_TMP}.otc_t_ext_terminales_ajst_tmp;
